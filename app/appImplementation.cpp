...
float *getObjectPose(InputArray frame, int * segmentation_values)
{
    ...
    {
        printf("size: %lu \n", ballsBox.size());
        pose[0] = 127 * (ballsBox[0].center.x  / 1024);    
        pose[1] = 127 * (ballsBox[0].center.y  / 768);   
        pose[2] = 127* ((ballsBox[0].angle + 360) / (720));
        
        if(ballsBox.size()> 1)
        {
            
            pose[3] = 127 * (ballsBox[1].center.x  / 1024);    
            pose[4] = 127 * (ballsBox[1].center.y  / 768);   
            pose[5] = 127* ((ballsBox[1].angle + 360) / (720));
        }
